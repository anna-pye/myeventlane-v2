...
/**
 * Queue worker for 2-hour reminder notifications.
 *
 * @QueueWorker(
 *   id = "automation_reminder_2h",
 *   title = @Translation("2-hour reminder notification worker"),
 *   cron = {"time" = 30}
 * )
 */
final class Reminder2hWorker extends AutomationWorkerBase {
  ...
  public function processItem($data): void {
    ...
    if (!$dispatchId || !$eventId || !$recipientEmail) {
      $this->logger->error('Reminder2hWorker: Missing required data');
      return;
    }
    ...
    // Send email.
    try {
      $this->messagingManager->queue('event_reminder_2h', $recipientEmail, $context);
      ...
      $this->logger->info('Sent 2h reminder for event @id to @email', [
        '@id' => $eventId,
      ...
      $this->logger->error('Failed to send 2h reminder: @message', ['@message' => $e->getMessage()]);
    }
  }
